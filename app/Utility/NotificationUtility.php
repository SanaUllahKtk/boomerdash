...
use App\Mail\InvoiceOrderManager;
use App\Mail\ConversionManager;
use App\Models\User;
...
class NotificationUtility
{
    ...
    public static function sendOrderStatusNotification($order, $request = null)
    {       
        //sends email to customer with the invoice pdf attached
        $reasion="";
        if($order->delivery_status=='cancelled'){
            $reasion= ucfirst($order->rejection).' Receipt';
            $status= 'Rejected';
        }
        ...
        $array['view'] = 'emails.invoice';
        $array['subject'] = translate('Your Order Status has been ') .' '. ucfirst($status) .' '.$reasion.' - ' . $order->code;
        $array['from'] = env('MAIL_FROM_ADDRESS');
        $array['reasion'] =  translate('Your Order Status has been ') .' '. ucfirst($status) .' '.$reasion.' - ' . $order->code;
        $array['order'] = $order;
        // try {
            Mail::to($order->user->email)->queue(new InvoiceOrderManager($array));
            Mail::to($order->orderDetails->first()->product->user->email)->queue(new InvoiceOrderManager($array));
        // } catch (\Exception $e) {
    ...
    public static function sendConversionNotification($order, $user)
    {   
        $array['view'] = 'emails.conversion';
        $array['subject'] = translate('Points Convert Alert');
        $array['from'] = env('MAIL_FROM_ADDRESS');
        $array['order'] = $order;
        // try {
            Mail::to($user->email)->queue(new ConversionManager($array));
            // Mail::to($order->orderDetails->first()->product->user->email)->queue(new ConversionManager($array));
        // } catch (\Exception $e) {
